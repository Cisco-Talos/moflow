...
(* Do not emit memory related asserts *)
let assert_only_jumps = ref true
...
let just_comments block = 
  let cmts = List.for_all is_comment block in 
  cmts
...
let rec get_tid stmts =
  ...
  let rec aux stmts = 
    ...
    | (Ast.Comment (c, _))::rest -> aux rest
    | _ ->  
      let _ = assert ((List.length stmts) > 0) in
      let hd = List.hd stmts in
      get_tid_from_atts (get_attrs hd)
  in
  (* Detect if block is 100% comments and return tid=0. *)
  if just_comments stmts then 
    Some 0
  else
    aux stmts
...
let trace_dce trace =
  ...
  (* let final = List.filter (fun s -> not (is_comment s)) final in *)
  Status.stop () ;
  final
...
let trace_transform_stmt stmt evalf =
  ...
  let update_exp cidx idx = 
    if not !assert_only_jumps then
  ...
  *)
  ...
  let addr = 
    ...
    (* SWXXX Verify everything in block is label or comment. 
     ...
     | Not_found -> 
       let _ = assert (List.length block > 0) in
       List.hd block
    )
  in
