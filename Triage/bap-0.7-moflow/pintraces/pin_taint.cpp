...
context TaintTracker::getMemory()
{
  return memory;
}
...
bool TaintTracker::taintPreSC(uint32_t callno, const uint64_t *args, /* out */ uint32_t &state)
{
  ...
  switch (callno) {
	...
	if (state == __NR_open) {
      ...
      case __NR_mmap:
      case __NR_mmap2:
        // fopen(fn, "rb") ends up calling mmap with PROT_WRITE bit set, so we
        // can't disallow it.
      ...
      case __NR_write:
        if (fds.find(args[0]) != fds.end()) {
          cerr << "Writing to tainted files is not supported" << endl;
          assert(false);
        }
        break;
  ...
  case __NR_mapviewofsectionwin:
    ...
    if(args[9] & _PROT_WRITE_WIN){
      cerr << "Mapping writable section views of tainted files is not \ 
              supported" << endl;
      assert(false);
    }
