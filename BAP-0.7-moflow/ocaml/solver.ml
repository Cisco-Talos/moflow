...
struct
  ...
  let mk_bv_0 ctx = fst (mk_bv_01 ctx)
  let mk_bv_1 ctx = snd (mk_bv_01 ctx)

  let bv_to_bool_exp ctx v = 
    let tr = mk_bv_1 ctx in
    mk_eq ctx v tr

  let bool_to_bv ctx cond invert =
    let fl, tr = mk_bv_01 ctx in
  ...
  let convert_cast {ctx = ctx} e (t,tnew) =
    (* Printf.printf "increasing bit size from %d to %d\n" (get_bits t)
    (get_bits tnew); 
    let _ = flush stdout in
    *)
    function
      | _ when tnew = t -> e
      (* Someone is using CAST_UNSIGNED instead of CAST_LOW :( *)
      | CAST_UNSIGNED -> 
        let delta = (get_bits tnew - get_bits t) in
        if delta > 0 then
          mk_zero_ext ctx delta e
        else
          assert false
  ...
  let rec convert ({ctx=ctx; vars=vars} as set) =
    ...
    | Let(v, e, e') ->
        ...
        e'
    | Extract(bigHi, bigLow, e) ->
        let e = convert set e in
        let hi = Big_int_Z.int_of_big_int bigHi in
        let lo = Big_int_Z.int_of_big_int bigLow in
        mk_extract ctx hi lo e
    | Concat(e1, e2) ->
        let e1 = convert set e1 in
        let e2 = convert set e2 in
        mk_concat ctx e1 e2
    | Ite(eif, et, ef) ->
        let eif = convert set eif in
        let eif = bv_to_bool_exp ctx eif in
        let et = convert set et in
        let ef = convert set ef in
        mk_ite ctx eif et ef
    | Unknown(_, Type.Reg(1)) ->
        (* Some flags are undefined after certain instructions. Treat them all
         * as 0. *)
        mk_bv_0 ctx
      ...
      | true, v ->
          (
            match get_ast_kind ctx v with
              ...
              (* This way we can recognize variables unbound in the model *)
              | APP_AST -> (false, 0x666)
