  ...
  let load num t bytes = 
    let rec load_aux acc shift = function
      ...
      | n -> 
          ...
          let acc = BinOp (OR, BinOp(LSHIFT, var, Int(big_int_of_int64 shift, t)), acc) in
  ...
  let break_move num value t bytes =
    let rec break_aux acc shift = function
      ...
      | n ->
          ...
          let te = Typecheck.infer_ast value in
          let exp = BinOp (RSHIFT, value, Int(big_int_of_int64 shift, te)) in
          let exp = Cast(CAST_LOW, reg_8, exp) in
          let acc = (Move(var, exp, [])) :: acc in
            break_aux acc (Int64.add shift 8L) (n - 1)
    in
    let var = newvar num reg_8 in
    let exp = Cast(CAST_LOW, reg_8, value) in
...
let run_and_subst_block state memv thread_map block = 
  ...
  let block, extra =
    let assigns = assign_vars memv (lookup_thread_map thread_map (get_tid
    [addr])) false in
    (* List.iter *)
  ...
  let rec eval_block state stmt = 
    ...
    (* Append stmts generated from Context attribute. This turns 
     * -trace-concrete-subst into -trace-concrete with vars instead of memory
     * references :). *)
    if !place >= 0 then (
      (* trace_transform_stmt2 removes writes to tainted vars/mem *)
      let trans = trace_transform_stmt2 stmt evalf in
      executed := BatList.append trans !executed ; 
    );
    place := !place + 1;
